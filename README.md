...
本项目把泌尿系统肿瘤 PET/CT 报告转换为患者容易理解的医学信息图。当前代码包含图片生成、AI 审查与修订、医生评价、患者理解度题目、批量实验审计和统计分析工具。

项目仍在开发和研究方案迭代中，正式 300 例研究尚未提交或完成。每个实验批次会锁定当时的数据、模型、提示词和参数；需要比较新模型或新提示词时，另建实验 ID。
...
这些 ID 用来把版本差异留在不同批次里。若增加第三种正式策略，还要同步扩展工作项定义、导出表、SAP 和统计方法。
