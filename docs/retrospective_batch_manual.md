...
本手册记录 API 配置、实验准备、小规模试跑、断点续跑、人工评价、数据导出和统计分析流程。项目尚未投稿，也未最终锁库；提示词、模型、参数和研究比较仍可调整。文中说的“锁定”只约束一个具体实验 ID，不影响后续创建新版本。
...
开发阶段仍可改模型、提示词或参数，但不要混在同一个实验 ID 里：

- 同一个 `experiment-id` 代表同一套不可变设置，只用于断点续跑；
- 模型、提示词、参考图、参数、代码或数据变更后，使用新的 `experiment-id`；
- 后续比较模型、提示词或门控策略时，以不同实验 ID 作为批次边界。
